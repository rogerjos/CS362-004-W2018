 ...
 * Date: 18 Feb 2018
 * Course: CS362
 * Description: random test generator for villageEffect() in dominion.c
 * Card text:  "+1 Card
 				+2 Actions"
*******************************************************************************/
...
/*	playVillage()
	Puts a Village in player hand, 
	saves game state,
	then calls villageEffect() */
int playVillage(struct gameState *pre, struct gameState *post);

/*	testVillage()
	Tests villageEffect() by comparing game states */
int testVillage(struct gameState const *pre, struct gameState const *post, int *err); 
...
int main(int argc, char *argv[]) {
	...
	testMsg[5] = "Cards in deck + discard decremented by one";
...
int playVillage(struct gameState *pre, struct gameState *post) {
	...
	pre->hand[pre->whoseTurn][pre->handCount[pre->whoseTurn]] = village; // Put an adventurer into current player's hand
...
int testVillage(struct gameState const *pre, struct gameState const *post, int *err) {
	...
	// If top of pile is not Village then error		
	else if (!(post->playedCards[post->playedCardCount - 1] == village)) {
			err[1]++;
	}

	// Village should net no change cards to hand since Village itself is removed from hand 
	if (post->handCount[player] != pre->handCount[player]) {
		err[2]++;
	}

	// Village should net +2 actions, since we are calling only the village effect itself and not the code that handles decrementing actions 
	if (post->numActions != pre->numActions + 2) {
		err[3]++;
	}

	// If deck is not empty before Village then no shuffle: deck size decrements 1
	if (pre->deckCount[player] > 0) { 
		if (post->deckCount[player] != (pre->deckCount[player] - 1)) {
			err[4]++;
		}
	}
	// If deck is empty before Village then shuffle required: deck size + discard size decrements by 1
	else if ((post->deckCount[player] + post->discardCount[player]) != (pre->deckCount[player] + pre->discardCount[player] - 1)) { 
		err[5]++;
	}
	...
	memcpy(&(test.numActions), &(post->numActions), sizeof(int));	// numActions
